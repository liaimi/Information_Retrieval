...
def sortSingleQuery_extra(query, urls, features):
    scores = {}
...
#            swVec[i] = 1 + B * float(1)/(1 + swVec[i] - qLen)
            swVec[i] = 1 + B * float(qLen)/swVec[i]
        
        nwVec = nmVec(query, features, url)
        for j in range(len(nwVec)):
            nwVec[j] = 1 + log(1 + nwVec[j])
        ...
        score = dot(QueVec, (swVec[0] * nwVec[0] * cu_sw * tf_url + swVec[1] * nwVec[1] * ct_sw * tf_title + swVec[2] * nwVec[2] *  cb_sw * tf_body + swVec[3] * nwVec[3] *  ch_sw * tf_header + swVec[4] * nwVec[4] * ca_sw * tf_anchor))
        scores[url] = score

    return list(sorted(scores, key = lambda x: scores[x], reverse = True))

def extraMethod(queries, features):
    rankedQueries = {}
    for query in queries:
        rankedQueries[query] = sortSingleQuery_extra(query, queries[query], features)
    return rankedQueries
...
def main(featureFile):
    ...
    rankedQueries = extraMethod(queries, features)
